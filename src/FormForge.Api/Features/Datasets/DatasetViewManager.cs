...
//
// Story 12.6 — the backing VIEWs live in the requesting tenant's own `{schema}_datasets`
// namespace (Story 12.7's naming), not the global `datasets` schema; ITenantContext
// resolves which one at call time (never cached — this instance is Scoped, matching
// ITenantContext's own lifetime). Falls back to the legacy `datasets` schema when no
// tenant is resolved. The resolved schema is re-validated via SafeIdentifier (defense-
// in-depth, same posture as TenantOnboardingService) before ever being interpolated.
[System.Diagnostics.CodeAnalysis.SuppressMessage("Performance", "CA1812",
    Justification = "Registered via DI.")]
internal sealed partial class DatasetViewManager(ITenantContext tenantContext, ILogger<DatasetViewManager> logger)
{
    // The tenant's own `{schema}_datasets` namespace, or the legacy global `datasets`
    // schema when no tenant is resolved.
    ...
    internal string BuildCreateViewDdl(DatasetName name, string effectiveQuery) =>
        $"""CREATE VIEW "{DatasetsSchema}"."{name.Value}" AS {effectiveQuery}""";
        ...
        CancellationToken ct)
    {
        ...
        LogViewCreated(logger, DatasetsSchema, name.Value);
    }

    [LoggerMessage(Level = LogLevel.Information, Message = "Created VIEW \"{Schema}\".\"{Name}\"")]
    private static partial void LogViewCreated(ILogger logger, string schema, string name);
    ...
    internal string BuildReplaceViewDdl(DatasetName name, string effectiveQuery) =>
        $"DROP VIEW IF EXISTS \"{DatasetsSchema}\".\"{name.Value}\";\n" +
        $"CREATE VIEW \"{DatasetsSchema}\".\"{name.Value}\" AS {effectiveQuery}";
    ...
    internal string BuildRenameViewDdl(string oldName, string newName) =>
        $"ALTER VIEW \"{DatasetsSchema}\".\"{oldName}\" RENAME TO \"{newName}\"";
        ...
        CancellationToken ct)
    {
        ...
        LogViewReplaced(logger, DatasetsSchema, name.Value);
    }
        ...
        CancellationToken ct)
    {
        ...
        LogViewRenamed(logger, DatasetsSchema, oldName, newName);
    }

    [LoggerMessage(Level = LogLevel.Information, Message = "Replaced VIEW \"{Schema}\".\"{Name}\"")]
    private static partial void LogViewReplaced(ILogger logger, string schema, string name);

    [LoggerMessage(Level = LogLevel.Information, Message = "Renamed VIEW \"{Schema}\".\"{OldName}\" → \"{NewName}\"")]
    private static partial void LogViewRenamed(ILogger logger, string schema, string oldName, string newName);
    ...
    internal string BuildDropViewDdl(string datasetName) =>
        $"DROP VIEW IF EXISTS \"{DatasetsSchema}\".\"{datasetName}\"";
        ...
        CancellationToken ct)
    {
        ...
        LogViewDropped(logger, DatasetsSchema, datasetName);
    }

    [LoggerMessage(Level = LogLevel.Information, Message = "Dropped VIEW \"{Schema}\".\"{Name}\"")]
    private static partial void LogViewDropped(ILogger logger, string schema, string name);
}
